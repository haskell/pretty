...
--
-- Here we expose the internal details for advance users such as GHC.
--
-----------------------------------------------------------------------------
...
data TextDetails = Chr  {-# UNPACK #-} !Char
                 ...
                 | PStr FastString -- ^ Used to represent a Fast String fragment
...
-- GHC: aboveNest :: RDoc -> Bool -> FastInt -> RDoc -> RDoc
...
renderStyle (Style m lineLen ribbons) doc
  = fullRender m lineLen ribbons txtToStr "" doc
...
showDocWithAppend m doc rest
  = fullRender m (lineLength style) (ribbonsPerLine style) txtToStr rest doc

txtToStr :: TextDetails -> String -> String
txtToStr (Chr c)     s  = c:s
txtToStr (Str s1)    s2 = s1 ++ s2
txtToStr (PStr s1)   s2 = unpackFS s1 ++ s2
txtToStr (LStr s1 _) s2 = unpackLitString s1 ++ s2
...
easy_display nl_space_text txt end doc
  ...
  where
    ...
    lay (Union _p q)       = lay q -- Second arg can't be NoDoc
-- XXX: What to do here?
    -- lay (Union _p q)        _   = lay (frist p q)
    lay (Nest _ p)         = lay p
    lay Empty              = end
